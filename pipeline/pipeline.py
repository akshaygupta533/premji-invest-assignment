...
def analyse_100k_dataset():
    ...
    if first_pipeline_completed:
        ...
        mean_occupation_age = analyzer.get_occupation_mean_age()
        #Task 2
        log.info("Doing task 2")
        top_20_movies = analyzer.get_top_n_movies(20,35)
        #Task 3
        log.info("Doing task 3")
        top_genres_occupation_and_age_group = analyzer.get_top_genres_occupation_and_age_group()
        #Task 4
    else:
        log.info("Skipping second pipeline because the first failed or did not complete.")
