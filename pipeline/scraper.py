...
class WebScraper:
    ...
    def get_yourstory_top_5(self,keyword):
        ...
        self.driver.get(url)
        elements = self.driver.find_elements(By.XPATH, "//div//li//span[@pathname='/search']")[:5]
        links = [
            el.find_element(By.XPATH,"./ancestor::a").get_attribute('href') for el in elements
        ]
        return links
    ...
    def get_yourstory_article_sentiment_score(self,url):
        ...
        self.reinit_driver()
        self.driver.get(url)
        article_div = self.driver.find_element(By.XPATH, "//div[@id='article_container']")
        article_text = article_div.text
        cleaned_text = self.clean_text(article_text)
        res = requests.post(f"{DUMMY_API_URL}/get-senti-score/",json = {"text":cleaned_text})
    ...
    def get_finshots_top_5(self,keyword):
        ...
        search_button = self.driver.find_element(By.CSS_SELECTOR, f"a.{search_class_name}")
        search_button.click()
        input_field = self.driver.find_element(By.XPATH,"//input[@type='search']")
        input_field.send_keys(keyword)
        article_elements = self.driver.find_elements(By.XPATH, "//a[@class='c-search-result']")[:5]
        links = [
            el.get_attribute('href') for el in article_elements
        ]
        return links
