    ...
    try:
        ...
        for ticker in tickers:
            ...
            for link in yourstory_links:
                ...
                score = scraper.get_yourstory_article_sentiment_score(url=link) #calls the mock api
                scores.append({"ticker":ticker,"url": link, "score": score})
            for link in finshots_links:
                log.info(f"Getting score for {link}")
                score = scraper.get_finshots_article_sentiment_score(url=link) #calls the mock api
                scores.append({"ticker":ticker, "url": link, "score": score})
